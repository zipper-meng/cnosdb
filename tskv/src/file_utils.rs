...
lazy_static! {
    ...
    static ref MAL_FILE_NAME_PATTERN: Regex = Regex::new(r"_\d{6}\.mal").unwrap();
...
pub fn make_summary_file(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("summary-{:06}", id);
    path.as_ref().join(p)
}

/// Make a path for summary temporary file by it's directory.
pub fn make_summary_file_tmp(path: impl AsRef<Path>) -> PathBuf {
    path.as_ref().join("summary.tmp")
}
...
pub fn make_index_binlog_file(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("_{:06}.binlog", id);
    path.as_ref().join(p)
}
...
pub fn make_wal_file(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("_{:06}.wal", id);
    path.as_ref().join(p)
}
...
/// Make a path for MAL (meta write ahead log) file by it's directory and id.
pub fn make_mal_file(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("_{:06}.mal", id);
    path.as_ref().join(p)
}

/// Check a MAL file's name.
pub fn check_mal_file_name(file_name: &str) -> bool {
    MAL_FILE_NAME_PATTERN.is_match(file_name)
}

/// Make a path for TSM file by it's directory and id.
pub fn make_tsm_file_name(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("_{:06}.tsm", id);
    path.as_ref().join(p)
}
...
pub fn make_tsm_tombstone_file_name(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("_{:06}.tombstone", id);
    path.as_ref().join(p)
}

/// Make a path for TSM delta file by it's directory and id.
pub fn make_delta_file_name(path: impl AsRef<Path>, id: u64) -> PathBuf {
    let p = format!("_{:06}.delta", id);
    path.as_ref().join(p)
}

/// Get the file's path that has the maximum id of files in a directory.
pub fn get_file_with_the_max_id<F>(
    dir: impl AsRef<Path>,
    file_to_id: F,
) -> Option<(PathBuf, u64)>
where
