...
impl NullableDataBlock {
    ...
    /// indexes.
    pub fn time_range_by_range(&self, start: usize, end: usize) -> TimeRange {
        (
...
impl From<DataBlock> for NullableDataBlock {
    fn from(data_block: DataBlock) -> Self {
        let (timestamps, field_values, enc) = match data_block {
            DataBlock::U64 { ts, val, enc } => (ts, PrimitiveDataBlock::U64(val), enc),
            DataBlock::I64 { ts, val, enc } => (ts, PrimitiveDataBlock::I64(val), enc),
            DataBlock::Str { ts, val, enc } => (ts, PrimitiveDataBlock::Str(val), enc),
            DataBlock::F64 { ts, val, enc } => (ts, PrimitiveDataBlock::F64(val), enc),
            DataBlock::Bool { ts, val, enc } => (ts, PrimitiveDataBlock::Bool(val), enc),
        };
        Self {
            timestamps,
            field_values,
