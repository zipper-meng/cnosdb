...
impl std::fmt::Display for CompactReq {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.version.borrowed_tenant_database(),
            self.version.tf_id(),
            self.in_level,
            self.out_level,
            self.out_time_range,
...
pub mod test {
    ...
    pub use super::compact::test::{
        check_column_file, create_tskv_options, write_data_blocks_to_column_file,
    };
    ...
    use crate::tsm::test_helper::{write_data_blocks_to_tsm, write_to_tsm_tombstone_v2};
        ...
        async fn make_tsm_files(&self, version: &Version) {
            ...
            for level_sketch in self.levels.iter() {
                for file_sketch in level_sketch.2.iter() {
                    ...
                    write_data_blocks_to_tsm(&tsm_path, tsm_data.clone()).await.unwrap();
    ...
    async fn test_generate_version() {
        ...
        let storage_opt = create_tskv_options(dir.to_string(), true).storage.clone();
        ...
        assert_eq!(version.borrowed_tenant_database(), "dba");
