...
pub enum MetaError {
    #[snafu(display("The member {member_name} of tenant {tenant_name} already exists"))]
    #[error_code(code = 1)]
    MemberAlreadyExists {
        member_name: String,
        tenant_name: String,
    ...
    #[snafu(display("The member {member_name} of tenant {tenant_name} not found"))]
    #[error_code(code = 2)]
    MemberNotFound {
        member_name: String,
        tenant_name: String,
    ...
    #[snafu(display("The privilege {name} already exists"))]
    #[error_code(code = 3)]
    PrivilegeAlreadyExists { name: String },

    #[snafu(display("The privilege {name} not found"))]
    #[error_code(code = 4)]
    PrivilegeNotFound { name: String },

    #[snafu(display("The role {role} already exists"))]
    #[error_code(code = 5)]
    RoleAlreadyExists { role: String },

    #[snafu(display("The role {role} not found"))]
    #[error_code(code = 6)]
    ...
    #[snafu(display("The tenant {tenant} already exists"))]
    #[error_code(code = 9)]
    TenantAlreadyExists { tenant: String },

    #[snafu(display("The tenant {tenant} not found"))]
    #[error_code(code = 10)]
    ...
    #[snafu(display("Database {database:?} already exists"))]
    #[error_code(code = 18)]
    ...
    #[snafu(display("Table {table_name} already exists."))]
    #[error_code(code = 20)]
    ...
    #[snafu(display("{kind} reached limit"))]
    #[error_code(code = 24)]
    RequestLimit { kind: RequestLimiterKind },

    #[snafu(display("An error occurred while processing the data: {msg}. Please try again"))]
    #[error_code(code = 25)]
    Retry { msg: String },
    ...
    #[snafu(display("Connect to Server({addr}) error reason: {msg}"))]
    #[error_code(code = 26)]
    ...
    #[snafu(display("The bucket {id} not found"))]
    #[error_code(code = 31)]
    BucketNotFound { id: u32 },

    #[snafu(display("database {name} attribute invalid!"))]
    #[error_code(code = 32)]
    DatabaseSchemaInvalid { name: String },

    #[snafu(display("update table {name} conflict"))]
    #[error_code(code = 33)]
    ...
    #[snafu(display("The vnode {id} not found"))]
    #[error_code(code = 37)]
    VnodeNotFound { id: u32 },

    #[snafu(display("Valid node is not enough, need: {need}, but found: {valid_node_num}",))]
    #[error_code(code = 53)]
