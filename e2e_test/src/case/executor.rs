...
impl E2eExecutor {
    ...
    pub fn execute_steps_legacy(&self, steps: &[StepLegacy]) {
        ...
        if self.is_singleton {
            ...
            self.execute_steps_legacy_inner(&mut None, &mut data, steps);
        } else {
            let (mut meta, data) = self.install_cluster();
            let mut data = data.unwrap();
            self.execute_steps_legacy_inner(&mut meta.as_mut(), &mut data, steps);
        }
        println!("Test complete: {}.{}", self.case_group, self.case_name);
    }

    #[deprecated]
    fn execute_steps_legacy_inner(
        &self,
        ...
        for (i, step) in steps.iter().enumerate() {
            ...
            match step {
                    ...
                    match req {
                        ...
                        CnosdbRequest::Write { url, req, resp } => {
                            ...
                            match resp {
                                Ok(exp_line) => {
                                    assert!(result_resp.is_ok(), "{fail_message}");
                                    if let Ok(resp_lines) = result_resp {
                                        assert_eq!(&resp_lines[0], exp_line, "{fail_message}");
                                    } else {
                                        assert!(result_resp.is_err(), "{fail_message}");
                                    }
                                }
                ...
                StepLegacy::StopMetaNode(meta_node_index) => {
...
impl std::fmt::Display for StepLegacy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            StepLegacy::StopMetaNode(i) => write!(f, "StopMeta of meta node {i}"),
        }
    }
}
